...
def test_crawl_all_stops_programme_details_on_cloudflare_hard_block(monkeypatch, capsys):
    programmes = [
        Programme(id=10, name="First", university_id=1, source_url="https://example.test/p/10"),
        Programme(id=20, name="Blocked", university_id=1, source_url="https://example.test/p/20"),
        Programme(id=30, name="Never", university_id=1, source_url="https://example.test/p/30"),
    ]
    calls = []

    monkeypatch.setattr(crawl_all_module, "polite_sleep", lambda: None)
    monkeypatch.setattr(crawl_all_module, "crawl_universities", lambda country, limit: 0)
    monkeypatch.setattr(crawl_all_module, "_load_universities", lambda country, limit: [])
    monkeypatch.setattr(crawl_all_module, "_load_uncrawled_programmes", lambda country, limit: programmes)

    def fake_crawl_programme_detail(programme_id: int, record_failure: bool = True) -> bool:
        calls.append(programme_id)
        if programme_id == 20:
            raise crawl_all_module.CloudflareHardBlockError("Cloudflare hard block detected")
        return True
    ...
    result = crawl_all_module.crawl_all(
        ...
        skip_programmes=True,
    )

    assert calls == [10, 20]
    assert result.details_success == 1
    assert result.details_failed == 1
    assert "stopping due to Cloudflare hard block" in capsys.readouterr().out
